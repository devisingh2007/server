...
app.get('/', (req, res) => {
    res.status(200).json({ message: "Your assignment 3 server is running" });
});
...
app.get('/states/highest-gdp', (req, res) => {
    try {
        if (!states || states.length === 0) {
            return res.status(404).json({ error: "No states data available" });
        }
...
app.patch('/states/:id/literacy', (req, res) => {
    ...
    const stateIndex = states.findIndex(p => p.id == ID);
    if (stateIndex !== -1) {
...
app.patch('/states/:id/gdp', (req, res) => {
    ...
    if (stateIndex !== -1) {
        const { gdp } = req.body;
        if (gdp !== undefined) states[stateIndex].gdp = gdp;
        res.status(200).json(states[stateIndex]);
    } else {
        res.status(404).json({ message: "State not found" });
    }
...
app.patch('/states/:id', (req, res) => {
    ...
    if (stateIndex !== -1) {
        ...
        if (name !== undefined) states[stateIndex].name = name;
        if (population !== undefined) states[stateIndex].population = population;
        if (literacyRate !== undefined) states[stateIndex].literacyRate = literacyRate;
        if (annualBudget !== undefined) states[stateIndex].annualBudget = annualBudget;
        if (gdp !== undefined) states[stateIndex].gdp = gdp;
...
app.delete('/states/:id', (req, res) => {
    ...
    if (stateIndex !== -1) {
        ...
        res.status(204).send().json({ message: "State deleted successfully" });
    } else {
        res.status(404).json({ message: "State not found" });
    }
});
...
app.delete('/states/name/:stateName', (req, res) => {
    ...
    if (stateIndex !== -1) {
        ...
        res.status(200).json(deletedState[0]);
    } else {
        res.status(404).json({ message: "State not found" });
    }
});
...
app.delete('/states/low-literacy/:percentage', (req, res) => {
    ...
    const lowLiteracyStates = states.filter(s => s.literacyRate < percentage);

    if (lowLiteracyStates.length > 0) {
        let count = 0;
        ...
        res.status(404).json({ message: "No low literacy states found" });
    }
});
